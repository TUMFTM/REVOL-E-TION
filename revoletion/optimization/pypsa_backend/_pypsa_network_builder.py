...
class PypsaNetworkBuilder:
    ...
    def build(self) -> pypsa.Network:
        self._apply_arrow_string_array_fix()
        return self._net
    ...
    def _apply_arrow_string_array_fix(self):
        """Convert ArrowStringArray columns/indices to regular object dtype."""
        # This fix necessary to support newer pandas versions (>=2.2) in PyPSA.
        # The `ArrowStringArray` as an index might lead to PyPSA being unable
        #  to construct the optimization model.
        # TODO: remove once https://github.com/PyPSA/PyPSA/issues/1585 is addressed in PyPSA.

        for component in self._net.components:
            df = component.static
            if not df.empty:
                if isinstance(df.index.values, pd.arrays.ArrowStringArray):
                    component.static.index = pd.Index(df.index.astype(object))
                for col in df.columns:
                    if isinstance(df[col].values, pd.arrays.ArrowStringArray):
                        component.static[col] = df[col].astype(object)
            for key in component.dynamic:
                dyn_df = component.dynamic[key]
                if isinstance(dyn_df, pd.DataFrame) and not dyn_df.empty:
                    if isinstance(dyn_df.columns.values, pd.arrays.ArrowStringArray):
                        component.dynamic[key].columns = pd.Index(dyn_df.columns.astype(object))
