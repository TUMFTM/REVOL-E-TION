...
class PyPSABlockVisitor(blocks.BlockVisitor[None]):
    ...
    def visit_grid_market(self, block: blocks.GridMarket, builder: PyPSANetworkBuilder, bus_connected: str) -> None:
        ...
        builder.add_generator(
            ...
            p_max_pu=1.0 if block.pwr_g2s is not None else np.nan,
        ...
        builder.add_generator(
            ...
            p_max_pu=1.0 if block.pwr_s2g is not None else np.nan,
    ...
    def visit_stationary_battery(
        ...
        max_charge_power_w = block.sizes["storage"].preexisting * block.crate_chg * block.eff["chg"]
        ...
        builder.add_link(
            ...
            p_nom=max_charge_power_w,
            marginal_cost=inflow_variable_costs,
            efficiency=block.eff["chg_int"],
        )

        max_discharge_power_w = block.sizes["storage"].preexisting * block.crate_dis * block.eff["dis"]
