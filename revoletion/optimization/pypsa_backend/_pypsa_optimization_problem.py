...
from ._pypsa_energy_system_constructor import PypsaEnergySystemConstructor, make_pypsa_label
...
class PypsaOptimizationProblem(optimization_problem.OptimizationProblem):
    def __init__(
        ...
        if self._config.warmstart:
            self._model = self._energy_system.optimize.create_model(include_objective_constant=False)
    ...
    def from_revoletion_scenario(
        ...
        constructor = PypsaEnergySystemConstructor(
            horizon,
            logger,
            config.cost_eps,
        ...
        pypsa_energy_system = constructor.create_pypsa_network(scenario.block_registry)

        return cls(pypsa_energy_system, logger, config)

    @override
    def solve(self) -> tuple[optimization_problem.OptimizationStatus, optimization_problem.OptimizationResult | None]:
        dti = self._energy_system.snapshots
        ...
        return status, PypsaOptimizationResult(net=self._energy_system)
    ...
    def solve_time_step(
        ...
        index_loc = self._energy_system.snapshots.get_loc(normalized_time_step)
        # The initial SoCs must not be updated for the first time step.
        if index_loc > 0:
            prev_time_step = self._energy_system.snapshots[index_loc - 1]
            # Only process stores if there are any.
            if not self._energy_system.stores.empty:
                self._energy_system.stores.e_initial = self._energy_system.stores_t.e.loc[prev_time_step]

            # Only process storage units if there are any.
            if not self._energy_system.storage_units.empty:
                self._energy_system.storage_units.state_of_charge_initial = (
                    self._energy_system.storage_units_t.state_of_charge.loc[prev_time_step]
                )
    ...
    def _do_solve(self, dti: pd.DatetimeIndex) -> optimization_problem.OptimizationStatus:
        ...
        if self._model:
            self._energy_system._model = self._model
            with suppress_output():
                # Solve the created model.
                solver_status_str, termination_condition_str = self._energy_system.optimize.solve_model(
                    **optimize_kwargs
                )
            del self._energy_system.model
        else:
            with suppress_output():
                solver_status_str, termination_condition_str = self._energy_system.optimize(dti, **optimize_kwargs)
