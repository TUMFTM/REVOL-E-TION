...
class BatteryPackModel:
    ...
    def __init__(self, scenario, commodity):
        ...
        if self.chemistry == 'nmc':
            ...
            self.data_path = os.path.join(self.scenario.run.path_pkg, 'input', 'battery_cells', 'sanyo_ur18650e.pkl')
            ...
            self.data_path = os.path.join(self.scenario.run.path_pkg, 'input', 'battery_cells', 'sony_us26650.pkl')
    ...
    def age(self, run, horizon):
        ...
        if isinstance(self.commodity.temp_battery, str):
            try:
                ...
                run.logger.warning(f'Scenario {self.scenario.name}: Battery temp source for storage'
                                   f' {self.commodity.name} not found, using 25°C default')
